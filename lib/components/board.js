...
const openBoardPopout = ({ version, seed, mode }) => {
  ...
  openPopout(
    ...
    settings.boardCellWidth * 7 + settings.boardHeaderWidth,
    settings.boardCellHeight * 7 + settings.boardHeaderHeight * 3
  );
};
...
const template = ({ version, seed, mode, filename }) => `
  <div class="bingo bingo-board table ${mode}">
    ...
    <div id="col6" class="header">${localize("COL6")}</div>
    <div id="col7" class="header">${localize("COL7")}</div>
...
export const renderBoard = ({ board, seed, mode, filename, version }) => {
  ...
  bltrHeader.addEventListener("click", () => {
    openRowPopout("BL-TR", board.map((row, j) => row[6 - j]).reverse());
  });
